...
| --- | --- |
| **8 native/ACP runtime integrations** | Keep the subscriptions and harnesses you already use. |
...
Projects are bound to real folders. Claude Code, Codex, Cursor, Antigravity, Grok, MiMo, MiniMax, and OpenCode each run through their own adapter and retain separate native sessions. The active provider, model, effort, permissions, execution state, duration, and available usage data remain visible in the product.
...
| Runtime | Required executable | Authentication owner | Per-turn tokens | Subscription quota |
| --- | --- | --- | --- | --- |
| Claude Code | `claude` | Claude CLI | Numeric when stream-json reports usage | Native collector when exposed |
| Codex | `codex` | Codex app-server | Numeric when app-server reports usage | Native collector when exposed |
| Cursor | `cursor-agent` | Cursor browser subscription | Numeric when the result reports usage | CLI/version-dependent |
| Antigravity | `agy` | Antigravity browser subscription | Unavailable; the CLI does not expose it reliably | CLI/version-dependent |
| Grok | `grok` | Grok browser subscription | Numeric when the end event reports usage | CLI/version-dependent |
| MiMo | `mimo` | MiMo token-plan credential | Numeric when step-finish reports usage | Web console only |
| MiniMax | `mmx` | MiniMax token-plan credential | Numeric when JSON output reports usage | CLI/version-dependent |
| OpenCode | `opencode acp` | Provider selected inside OpenCode | ACP reports context occupancy, not turn spend | Owned by the selected provider |

Removing an executable marks that runtime unavailable; it does not delete the
project, task, conversation, worktree, encrypted database, or native-session
reference. Reinstalling and authenticating the executable restores runtime
availability.
